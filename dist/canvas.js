...
//Creates canvas
var makeUserCanvas = function (ptnl_input, stat_input, part_input, auth_input, dark, ideology) {
    ...
    if (part > 30) {
        ctx.fillText(partisan + "%", 150, 477.5);
    }
    if (auth > 30) {
        ctx.fillText(autocracy + "%", 150, 597.5);
    }
    ...
    if (part < 70) {
        ctx.fillText(nonpartisan + "%", 650, 477.5);
    }
    if (auth < 70) {
        ctx.fillText(democracy + "%", 650, 597.5);
    }
